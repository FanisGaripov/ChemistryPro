...
INIT_URL = BASE_URL + "index.php?proj=EA45D8517ABEB35140D0D83E76F14A41"
...
params = {
    "proj": "EA45D8517ABEB35140D0D83E76F14A41",
    "search": "1",
    "pagesize": "50",
    "theme": "",
    "qkind": "",
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if not task_elements and page == 0:
                raise ValueError(
                    "Не найдено ни одного задания на первой странице"
                )
            ...
            for task in task_elements:
                try:
                    ...
                    div_with_type_and_topic = soup.find(
                        "div", id=f"i{task_id}"
                    )
                    task_type = task.select_one("div.hint").get_text()
                    task_text = task.find("table").get_text()
                    script_text = task.find(
                        "script", string=re.compile("ShowPictureQ")
                    )
                    match = re.search(
                        r"ShowPictureQ\('(.+?)'\)", str(script_text)
                    )
                    if match is not None:
                        ...
                        task_image = None
                    task_topic = div_with_type_and_topic.find(
                        "td", class_="param-row"
                    )
                    if task_topic is not None:
                        task_topic = task_topic.get_text()
                    else:
                        task_topic = None
            ...
            print(
                f"Добавлено {current_page_tasks} заданий со страницы {page + 1}"
            )
            ...
            if current_page_tasks == 0:
                empty_pages_count += 1
                print(
                    f"Пустая страница. Счетчик пустых страниц: {empty_pages_count}/{max_empty_pages}"
                )
            else:
                empty_pages_count = 0
            ...
            print(
                f"Ошибка при обработке страницы {page + 1} (попытка {attempt + 1}): {str(e)}"
            )
            if attempt == max_attempts - 1:
                has_more = False
                print(
                    "Превышено максимальное количество попыток. Завершаем сбор."
                )
                break
